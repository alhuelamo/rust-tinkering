...
impl<T: fmt::Display> LinkedList<T> {
    ...
    pub fn append(self, element: T) -> Self {
        match self {
            ...
            LinkedList::Tail { head } => LinkedList::Link { head, tail: Box::new(LinkedList::new_head(element)) },
            LinkedList::Link { head, tail } => LinkedList::Link {
                head,
                tail: Box::new(tail.append(element))
            }
        }
    }
